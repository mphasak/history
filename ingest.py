...
UPSERTS = {
    ...
    """,
    'geography': """
        INSERT INTO geography (id, display_name, type, centroid, parent_id)
...
def upsert(dsn: str, ds: Dataset, dry_run: bool = False) -> None:
    ...
    with psycopg.connect(dsn, autocommit=False) as conn:
        with conn.cursor() as cur:
            # Sources first
            for s in ds.sources:
                cur.execute(UPSERTS['source'], s)

            # Geographies (with computed centroid WKT)
            for g in ds.geographies:
                cur.execute(UPSERTS['geography'], {
                    **g, 'centroid': _point_wkt(g['lat'], g['lon']),
                })
            ...
            # TraitRelations: insert as a batch, claim_id linkage handled later
            cur.execute('DELETE FROM trait_relation WHERE id::text = ANY(%s)',
                        ([r['id'] for r in ds.trait_relations if r['id']],))
            for r in ds.trait_relations:
                cur.execute("""
            ...
            cur.execute("""DELETE FROM carrier_trait_mix
                           WHERE (carrier_id, as_of_year, domain, trait_id) IN (
                             SELECT * FROM unnest(%s::text[], %s::int[],
                                                  %s::trait_domain[], %s::text[])
                           )""",
                        ([m['carrier_id'] for m in ds.carrier_trait_mix],
                         [m['as_of_year'] for m in ds.carrier_trait_mix],
                         [m['domain'] for m in ds.carrier_trait_mix],
                         [m['trait_id'] for m in ds.carrier_trait_mix]))
            for m in ds.carrier_trait_mix:
                cur.execute("""
            ...
            # PhysicalFeatures + snapshots (one snapshot per row in v0.3 spreadsheet)
            for f in ds.physical_features:
                cur.execute("""
            ...
            for c in ds.claims:
                cur.execute("""
                    ...
                    VALUES (%(subject_type)s, %(subject_id)s, %(statement)s,
                            %(quant_value)s, %(confidence)s, %(politically_sensitive)s)
                    RETURNING id;
            ...
            for e in ds.perspective_endorsements:
                ...
                cur.execute("""
                    ...
                    VALUES (%(perspective_id)s, %(subject_type)s, %(subject_id)s,
                            ...
                            %(override_quant_json)s, %(source_overrides_json)s);
                """, {
                    **e,
